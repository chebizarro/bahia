...
After the control-plane relays accept a self-issued `5401`, Bahia submits a Loom kind `5100` through the existing Loom client on the same relay pool and signer. The job includes `p=<trusted worker>`, `method=ci/workflow-run`, `e=<5401 event id>`, `run=<5401 event id>`, and the credential-free private mirror clone URL, requested ref, and workflow path. The request also has exactly two `secret` tags, `HIVE_CI_GIT_USERNAME` and `HIVE_CI_GIT_PASSWORD`, whose values are independently NIP-44 encrypted to the selected worker. Selection is restricted to `hiveci.trusted_loom_worker_pubkeys` and signed kind-`10100` advertisements containing both workload `ci/workflow-run` and feature `hive_ci_profile`; administrative labels are not capability evidence. Bahia resolves a dedicated, service-scoped read-only mirror credential before publishing and records only its opaque UUID in queued evidence.

This fleet-internal profile intentionally sends no `payment` tag because Bahia has no mint-backed Cashu payment capability and the worker authorizes unpaid CI requests by requester pubkey. Bahia also intentionally does not deliver `HIVE_CI_NSEC`: the `5401` publisher remains the Bahia service key, while a worker-signed `5402` is accepted only from `hiveci.trusted_loom_worker_pubkeys`. Any future move to per-run ephemeral publishers is a protocol change, not a compatibility fallback.
