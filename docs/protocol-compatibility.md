...
Bahia's signed inbound `build/request` is a ContextVM kind-`25910` mutation, but the private-mirror initiator publishes the resulting durable CI-bus event as kind `5401`. The self-dispatched event follows grasp-gitea's empty-content tag vocabulary and its id is returned as `ci_run_id` for 5402 lineage. Bahia then submits a kind-5100 job with `e=<ci_run_id>` and `method=ci/workflow-run` to a trusted worker advertising the matching workload and `hive_ci_profile` feature. The mirror URL stays credential-free; exactly `HIVE_CI_GIT_USERNAME` and `HIVE_CI_GIT_PASSWORD` are carried as NIP-44-encrypted `secret` tags bound to that selected worker, using a separately configured service-scoped read-only credential. The fleet-internal job omits payment because Bahia's Cashu adapter cannot mint, and it preserves Bahia's service publisher plus trusted worker-signed 5402 model instead of the per-run ephemeral-key model. Bahia's service pubkey remains an operator-managed `trusted_ci_pubkeys` entry; missing self-trust is warned, never auto-added.
