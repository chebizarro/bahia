...
For Bahia's private-mirror build path, `build/request` remains an inbound ContextVM method on kind `25910`. Once accepted, the initiator emits the CI-bus workflow run as a tag-only kind `5401` whose `publisher` is a per-run ephemeral key, returns its event id as `ci_run_id`, and submits a targeted loom-protocol kind `5100` carrying `cmd=loom-ci`, `args`, and `e=<ci_run_id>`. The selected trusted worker must advertise `loom-ci` as `S` software. The credential-free clone URL travels in `args`; exactly three NIP-44-encrypted secret tags — `HIVE_CI_GIT_USERNAME`, `HIVE_CI_GIT_PASSWORD`, and `HIVE_CI_NSEC` (the ephemeral publisher key) — are encrypted to that selected worker. This fleet-internal job has no payment tag; the worker signs the 5402 with the delivered publisher key. A 25910 command envelope is ephemeral and must not substitute for the durable 5401 event.
