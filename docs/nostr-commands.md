...
For Bahia's private-mirror build path, `build/request` remains an inbound ContextVM method on kind `25910`. Once accepted, the initiator emits the CI-bus workflow run as a tag-only kind `5401`, returns its event id as `ci_run_id`, and submits a targeted kind `5100` carrying `method=ci/workflow-run` and `e=<ci_run_id>`. The selected trusted worker must advertise workload `ci/workflow-run` and feature `hive_ci_profile`. Its credential-free clone URL is accompanied by exactly two NIP-44-encrypted secret tags for `HIVE_CI_GIT_USERNAME` and `HIVE_CI_GIT_PASSWORD`, encrypted to that selected worker from a dedicated service-scoped read-only mirror credential. This fleet-internal job has no payment tag and preserves Bahia's configured trusted-worker 5402 signer model rather than delivering an ephemeral publisher key. A 25910 command envelope is ephemeral and must not substitute for the durable 5401 event.
