...
Saving this section publishes a ContextVM kind `25910` intent with method `settings/relay-policy.apply`. Bahia validates the payload, updates the in-process policy snapshot for the explicit mutation path, republishes the standard service-signed NIP-51 relay events (`30002` for browser/ContextVM/service and `10050` for explicit notification DM relays), and publishes a canonical relay settings state event (`30900`, `d=relay-settings:operator`, `schema=bahia.relay-settings.v1`) plus an audit event (`4903`). Backend startup/live hydration and the Settings page subscribe to that canonical `30900` event with `#d`, `#domain`, `#schema`, and service-author filters, process EOSE as historical catch-up, and keep the subscription open for realtime policy convergence. Background hydration stores the latest canonical policy in a synchronized snapshot rather than mutating shared runtime config; atomic relay-pool reconfiguration is a separate runtime concern. If canonical state arrives while an operator has unsaved local edits in Settings, the page preserves those edits and shows explicit **Apply Canonical State** / **Keep Local Edits** actions instead of silently overwriting fields. Clients must treat the ContextVM response as an acknowledgment; durable UI truth comes from the service-signed relay-list and state/read-model events.
