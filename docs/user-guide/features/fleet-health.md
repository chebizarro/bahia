...
Bahia's own observables count too. The projector observes every validated, persisted event, including relay echoes of events this Bahia instance published. The projection is held in memory and is not replayed from relay history on restart, so after a restart subject gauges fill in as new observables arrive.
