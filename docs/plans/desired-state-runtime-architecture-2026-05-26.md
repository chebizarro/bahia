...
Per-service fragments and service-scoped Compose apply are deferred until full-project synthesis is stable. Phase 1 should keep the canonical generated Compose project as the only operational output; a later plan can decide fragment layout, eligibility rules, and operator visibility from the final renderer shape.
...
Per-service fragments and service-scoped Compose apply are intentionally deferred until phase 1 proves full-project synthesis, hashing, observation normalization, and environment locking in production-like staging. The follow-up plan should start from the final renderer shape and then decide fragment file layout, eligibility rules, and operator visibility. Until then, unsafe or ambiguous Compose changes should use full-project apply.
