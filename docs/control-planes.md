...
| SoulFactory lifecycle | Operator, SoulFactory controller, runtime sidecar | ContextVM `25910` methods `soul-factory/provision` and `soul-factory/action`; provisioning `30900`/`4903` | Canonical mutation entry point. The response is acceptance only; provisioning progress/outcomes project canonical state and audit while action and Soul read-model projection remain staged. |
