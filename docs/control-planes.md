...
| SoulFactory lifecycle | Operator, SoulFactory controller, runtime sidecar | ContextVM `25910` methods `soul-factory/provision` and `soul-factory/action` | Canonical mutation entry point. The response is acceptance only; existing correlated lifecycle events remain staged completion observables until `30900`/`4903` projection lands. |
