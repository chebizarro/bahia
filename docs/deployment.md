...
Reconcile policy is persisted on `environments.targeting.default_reconcile_mode` and, for explicit units, `deployment_units.reconcile_mode`. `observe_only`, `auto_apply`, and `approval_required` are active observation modes in this release: the scheduler records runtime observations and drift state but does not apply remediation. `disabled` excludes the environment or unit from scheduled observation. Explicit unit policy overrides the environment default; implicit default-unit rows use the environment default.
