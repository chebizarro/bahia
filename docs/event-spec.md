...
An operator's `build/request` remains a ContextVM kind-`25910` command. It is not the CI bus: the initiator's outbound workflow run is durable kind `5401` with empty content and `a`, `commit`, `branch`, `trigger`, `triggered-by`, `workflow`, `publisher`, and `t=hive-ci` tags. Its `publisher` tag is a per-run ephemeral key, as hive-ci-protocol requires. Its event id is both the 5402 lineage key and the `e` correlation (and `--run` argument) on the targeted kind-5100 Loom job. That job is loom-protocol shaped — `cmd=loom-ci`, `args=[run --repo … --ref … --workflow … --event push --actor … --run … --dep …]` — addressed to a trusted worker advertising `loom-ci` as `S` software, with credential-free mirror URL and exactly three selected-worker NIP-44-encrypted `secret` tags (`HIVE_CI_GIT_USERNAME`, `HIVE_CI_GIT_PASSWORD`, `HIVE_CI_NSEC`), and no payment tag. Operators must explicitly trust the Bahia service pubkey as a CI issuer; the 5402 is accepted because the worker signs it with the delivered ephemeral publisher key.
...
Canonical kind: `30315`.

- `domain=agent-runtime-release`: `d=runtime-release:<uuid>`; tags include `org`, `source`, `digest`, `branch`, and `release_channel`. Content includes the runtime repository and immutable producer provenance.
