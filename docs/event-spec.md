...
An operator's `build/request` remains a ContextVM kind-`25910` command. It is not the CI bus: the initiator's outbound workflow run is durable kind `5401` with empty content and `a`, `commit`, `branch`, `trigger`, `triggered-by`, `workflow`, `publisher`, and `t=hive-ci` tags. Its event id is both the 5402 lineage key and the `e`/`run` correlation on the targeted kind-5100 Loom job. That job uses `method=ci/workflow-run`, a capable trusted worker, credential-free mirror/ref/workflow parameters, exactly two selected-worker NIP-44-encrypted `secret` tags (`HIVE_CI_GIT_USERNAME` and `HIVE_CI_GIT_PASSWORD`), and no payment tag. Operators must explicitly trust the Bahia service pubkey as a CI issuer and the selected Loom worker as a 5402 signer; Bahia does not deliver a per-run ephemeral publisher key.
