...
| 1. Auth/governance staging | Bahia staging runs with `auth.enabled=true`, `adoption.enabled=true`, `direct_runtime_actions.enabled=true`, operator allowlists set, and `adoption.allow_raw_docker_hosts=false` | Non-operators receive 403; unauthenticated callers receive 401; operator flows reach handlers for the configured auth method. |
...
| Auth-enabled privileged routing | `go test ./internal/api/router -run 'TestPrivilegedRoutes|TestAdoptionRoutes'` | Disabled routes are absent; unauthenticated/non-operator JWT callers are rejected; JWT operator calls reach adoption handlers. NIP-98 operator routing remains a staging/live-network row when NIP-98 is the configured production auth method. | `TestPrivilegedRoutesDisabledByDefault`, `TestPrivilegedRoutesRequireOperatorAccess`, `TestAdoptionRoutesScanManagedEndpointsWithOperatorAuth` |
...
| Client contract | `go test ./pkg/client -run 'Adoption|RuntimeAction|Privileged'` | Client sends stable DTOs, preserves redaction metadata, sends bearer auth on adoption/import/runtime action calls, and decodes action responses. | `TestPrivilegedMethodsSendBearerToken`, adoption/import/runtime client tests |
...
| --- | --- | --- | --- | --- |
| LN-01 | Auth-enabled operator access | With the production auth method enabled (JWT and/or NIP-98), call scan/import/restart as no credential, a non-operator credential, and an operator credential. | 401 for no credential, 403 for non-operator, operator reaches the configured flow. For NIP-98 deployments, signed requests validate on the privileged routes. | Any unauthenticated or non-operator privileged action succeeds, or NIP-98 production auth cannot be validated end-to-end. |
