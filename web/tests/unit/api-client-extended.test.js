...
describe('BahiaClient - Extended API Coverage', () => {
  ...
  describe('Auth Exchange', () => {
    it('should call exchangeNostrAuth without Authorization header', async () => {
      // Set a token first
      api.setToken('existing-jwt-token');

      const event = {
        kind: 27235,
        content: '',
        tags: [['u', 'https://bahia.example.com/api/v1/auth/nostr']],
      ...
      const result = await api.exchangeNostrAuth(event);

      // Verify fetch was called without Authorization header
      const callArgs = global.fetch.mock.calls[0];
      expect(callArgs[0]).toBe('/api/v1/auth/nostr');
      expect(callArgs[1].method).toBe('POST');
      expect(callArgs[1].headers).not.toHaveProperty('Authorization');
      expect(callArgs[1].body).toBe(JSON.stringify({ event }));
      ...
      // Verify original token is still set after exchange
      expect(api.token).toBe('existing-jwt-token');
    });

    it('should handle exchangeNostrAuth when no token is set', async () => {
      const event = {
        kind: 27235,
        content: '',
        tags: [],
      ...
      global.fetch.mockResolvedValueOnce({
        ...
        headers: new Map([['content-type', 'application/json']]),
        json: async () => ({
          data: { token: 'first-jwt-token', pubkey: event.pubkey }
        })
      });

      const result = await api.exchangeNostrAuth(event);

      expect(result.token).toBe('first-jwt-token');
      expect(api.token).toBeNull();
    });

    it('should handle exchangeNostrAuth error with Bahia envelope', async () => {
      const event = {
        kind: 27235,
        content: '',
        tags: [],
      ...
      global.fetch.mockResolvedValueOnce({
        ok: true,
        headers: new Map([['content-type', 'application/json']]),
        json: async () => ({ error: 'Invalid event signature' })
      });

      await expect(api.exchangeNostrAuth(event)).rejects.toThrow('Invalid event signature');
    });

    it('should handle exchangeNostrAuth HTTP error', async () => {
      const event = {
        kind: 27235,
        content: '',
        tags: [],
      ...
      global.fetch.mockResolvedValueOnce({
        ...
        headers: new Map([['content-type', 'application/json']]),
        json: async () => ({ error: 'Event verification failed' })
      });

      await expect(api.exchangeNostrAuth(event)).rejects.toThrow('Event verification failed');
    });

    it('should restore saved token after exchangeNostrAuth failure', async () => {
      ...
      await expect(api.exchangeNostrAuth(event)).rejects.toThrow('Network error');

      // Verify original token is restored
      expect(api.token).toBe('original-token');
    });
  });
});
