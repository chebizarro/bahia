<script>
  ...
  const canPlay = $derived(!disabled && !playing && (spec.sample_text || '').trim().length > 0);
  ...
  <div class="studio-grid">
    <div class="form-stack">
      ...
      <label>
        ...
        <textarea rows="4" value={spec.sample_text || ''} disabled={disabled || playing} oninput={(event) => patch({ sample_text: event.currentTarget.value })} placeholder="Hello, I'm Scout. Let me help you find what you're looking for."></textarea>
      </label>
      <div class="action-row">
        <button type="button" class="btn-primary" disabled={!canPlay} onclick={playSample}>{playing ? 'Requesting…' : 'Play sample'}</button>
      </div>
    ...
    <aside class="settings-card">
      ...
      <label><span>Speed</span><input type="number" min="0.25" max="4" step="0.05" value={providerSettings.speed || 1} disabled={disabled || playing} oninput={(event) => patchProviderSettings({ speed: Number(event.currentTarget.value) || 1 })} /></label>
      ...
      {:else if selectedProvider === 'azure'}
        ...
        <label><span>Style degree</span><input type="number" min="0" max="2" step="0.1" value={providerSettings.style_degree || 1} disabled={disabled || playing} oninput={(event) => patchProviderSettings({ style_degree: Number(event.currentTarget.value) })} /></label>
...
<style>
  ...
  h3, h4, p { margin: 0; }
  p, .provider-list span { color: var(--text-muted); font-size: 0.85rem; }
  ...
  button { border: 1px solid var(--border-color); border-radius: 8px; background: transparent; color: var(--text-primary); padding: 0.55rem 0.8rem; font: inherit; cursor: pointer; }
  ...
  @media (max-width: 820px) { .studio-grid, .two-col, .three-col, .provider-list { grid-template-columns: 1fr; } .panel-header, .action-row { flex-direction: column; } }
</style>
