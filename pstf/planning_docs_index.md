...
|---|---|---|---|---|---|
| `README.md` | Product positioning and quick start | Core deployment story is still right; omits sidecar-first bootstrap, encrypted browser flows, MCP, and `/api/v1/system/info` importance | builds, artifacts, deploys, drift, rollback, Soul Factory, OCI, Hive-CI | conflicts with newer control-plane framing in `docs/control-planes.md` and observed web bootstrap | medium |
...
| `docs/control-planes.md` | Primary narrative spec for control-plane topology and transport semantics | matches router, system-info bootstrap, sidecar-first topology, encrypted request/result contract, signer-first operator flows, and 311xx deprecation | public request/result/read-model kinds, encrypted flows, adoption, direct runtime, LLM, MCP, sidecar | strongest narrative spec; only partial gap is that newer web helper kinds go beyond the summarized tables | high |
| `docs/api.md` | REST API reference | covers major CRUD surfaces but omits key mounted routes | services, environments, builds, artifacts, deployments, rollback, adoption, direct runtime, OCI | omits `/api/v1/system/info`, `/mcp`, orgs, workers, payments, notifications, signatures, SBOM, and some LLM/read-only routes present in `internal/api/router/router.go` | medium |
...
| `docs/relay-sidecar.md` | Relay sidecar topology and policy description | consistent with sidecar-first architecture and system-info relay discovery | public browser relays, sidecar/backend URLs, mirrored kinds, policy | narrow scope only; complements `docs/control-planes.md` | high |
...
5. `internal/api/handlers/system.go`
