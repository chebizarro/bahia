...
def projection_from_readiness(payload: dict[str, Any]) -> dict[str, str] | None:
...
def fetch_readiness(url: str) -> dict[str, Any]:
    try:
        ...
        # Older Bahia releases may return 503 solely because the optional
        # projection is absent. Parse the diagnostic body so a deployment can
        # transition away from that circular readiness dependency.
        raw = exc.read()
...
def capture(url: str, output: Path) -> None:
    ...
    baseline = {"present": projection is not None}
    if projection is not None:
        baseline["projection"] = projection
    output.write_text(json.dumps(baseline, sort_keys=True) + "\n", encoding="utf-8")
...
def verify(url: str, baseline_path: Path) -> None:
    ...
    if baseline.get("present") is not True:
        return
    before = baseline.get("projection")
    if not isinstance(before, dict):
        raise RolloutGateError("present rollout baseline has no projection")
    after = projection_from_readiness(fetch_readiness(url))
    if after is None:
        raise RolloutGateError("relay policy present before rollout is unavailable after rollout")
