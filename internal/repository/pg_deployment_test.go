...
func TestPgDeploymentIntentRepository_GetByHiveResultEventID(t *testing.T) {
	...
	mock.ExpectQuery("metadata->>'hive_ci_result_event_id' = \\$1").
		...
		WillReturnRows(pgxmock.NewRows([]string{"id", "service_id", "environment_id", "artifact_id", "requested_by", "source_kind", "approval_status", "status", "supersedes_intent_id", "approval_metadata", "metadata", "created_at", "approved_at", "updated_at"}).
			AddRow(id, svcID, envID, artifactID, "npub1xyz", "auto_promote", "approved", "pending", nil, []byte(`{"reviewer":"ops"}`), []byte(`{"hive_ci_result_event_id":"result-evt-123"}`), now, nil, now))
