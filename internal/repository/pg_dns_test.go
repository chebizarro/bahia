...
func TestPgDNSZoneRepositoryPersistsMeshVisibility(t *testing.T) {
	...
	zone := &domain.DNSZone{Name: "mesh.example", Visibility: domain.ZoneVisibilityMesh, BackendRef: "mesh-dns", TTL: 60}

	mock.ExpectExec("INSERT INTO dns_zones").WithArgs(zone.Name, domain.ZoneVisibilityMesh, zone.BackendRef, zone.TTL).WillReturnResult(pgconn.NewCommandTag("INSERT 0 1"))
...
func TestPgDNSZoneAndOverrideRepositoriesPersistAndListActive(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO dns_zones").WithArgs(zone.Name, zone.Visibility, zone.BackendRef, zone.TTL).WillReturnResult(pgconn.NewCommandTag("INSERT 0 1"))
