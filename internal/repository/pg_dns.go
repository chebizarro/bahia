...
func (r *PgDNSZoneRepository) Create(ctx context.Context, zone *domain.DNSZone) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO dns_zones (name, visibility, backend_ref, ttl)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (name) DO UPDATE SET visibility = EXCLUDED.visibility, backend_ref = EXCLUDED.backend_ref, ttl = EXCLUDED.ttl, updated_at = now()
	`, zone.Name, zone.Visibility, zone.BackendRef, zone.TTL)
	if err != nil {
...
func (r *PgDNSZoneRepository) Get(ctx context.Context, name string) (*domain.DNSZone, error) {
	zone := &domain.DNSZone{}
	err := r.pool.QueryRow(ctx, `SELECT name, visibility, backend_ref, ttl FROM dns_zones WHERE name = $1`, name).Scan(&zone.Name, &zone.Visibility, &zone.BackendRef, &zone.TTL)
	if err == pgx.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func (r *PgDNSZoneRepository) List(ctx context.Context) ([]domain.DNSZone, error) {
	rows, err := r.pool.Query(ctx, `SELECT name, visibility, backend_ref, ttl FROM dns_zones ORDER BY name`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&zone.Name, &zone.Visibility, &zone.BackendRef, &zone.TTL); err != nil {
