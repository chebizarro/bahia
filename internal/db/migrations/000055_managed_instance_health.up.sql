...
CREATE TABLE managed_instance_health (
  ...
  deployment_unit_id UUID NOT NULL REFERENCES deployment_units(id) ON DELETE CASCADE,
  ...
  PRIMARY KEY (service_id, environment_id, deployment_unit_id, runtime_target_name),
...
CREATE TABLE managed_instance_overrides (
  ...
  UNIQUE (service_id, environment_id, deployment_unit_id, runtime_target_name),
  CHECK (runtime_target_name <> ''),
  CHECK (actor <> ''),
  CHECK (reason <> ''),
