...
// creation on it, including deployments backed by a runtime release.
func (s *PolicyService) DeploymentApprovalRequired(ctx context.Context, environmentID uuid.UUID) (bool, error) {
	if s == nil || s.policies == nil {
		return false, fmt.Errorf("deployment approval policy repository is not configured")
	}
	globalPolicies, err := s.policies.ListGlobal(ctx)
	if err != nil {
