...
func TestPolicyService_DeploymentApprovalRequired(t *testing.T) {
	...
	if got, err := nilSvc.DeploymentApprovalRequired(ctx, envID); err == nil || got {
		t.Fatalf("nil service = %t, %v; want false and an error", got, err)
	}
}
...
func (s *stubApprovalPolicy) DeploymentApprovalRequired(context.Context, uuid.UUID) (bool, error) {
	s.calls++
