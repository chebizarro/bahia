...
func newLLMRegistryFakes(routeID, releaseID, envID uuid.UUID) llmRegistryFakes {
	...
	return llmRegistryFakes{routes: routes, releases: releases, envs: envs, intents: &fakeLLMIntentRepo{byID: map[uuid.UUID]*domain.LLMDeploymentIntent{}}, runs: &fakeLLMRunRepo{byID: map[uuid.UUID]*domain.LLMDeploymentRun{}}, obs: &fakeLLMObsRepo{}, states: &fakeLLMStateRepo{byKey: map[string]*domain.LLMRouteState{}}}
}
...
func (r *fakeLLMIntentRepo) ListByRouteEnv(context.Context, uuid.UUID, uuid.UUID, int, int) ([]domain.LLMDeploymentIntent, error) {
	return nil, nil
}
