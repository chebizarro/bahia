...
var constantJustifications = map[string]KindJustification{
	...
	"LegacyWorkerState":              omitted("LegacyWorkerState", kinds.LegacyWorkerState, "conflicting-alias", "shares canonical with SystemDiscovery; ResolveDisposition maps worker-tagged/worker-shaped events to worker state"),
...
func buildManifest() map[int]Disposition {
	...
	for _, parts := range requestMethods {
		addIntent(parts.kind, parts.domain, parts.op)
	}
