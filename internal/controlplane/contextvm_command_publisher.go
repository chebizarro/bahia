...
func publishContextVMCommand(ctx context.Context, publisher NostrEventPublisher, signer canonicalnostr.Signer, method, dTag, agentID string, tags nostr.Tags, params map[string]any, label string) (*nostr.Event, int, string, error) {
	...
	if method == "" {
		return nil, 0, "", fmt.Errorf("%s ContextVM method is required", label)
	}
	dTag = strings.TrimSpace(dTag)
	if dTag == "" {
		return nil, 0, "", fmt.Errorf("%s idempotency key is required", label)
	}
	...
	if err != nil {
		return nil, 0, dTag, fmt.Errorf("marshal %s ContextVM params: %w", label, err)
	}
	...
	if err != nil {
		return nil, 0, dTag, fmt.Errorf("marshal %s ContextVM request: %w", label, err)
	}
	...
	if err := SignGoNostrEvent(ctx, signer, ev); err != nil {
		return nil, 0, dTag, fmt.Errorf("sign %s ContextVM request: %w", label, err)
	}
