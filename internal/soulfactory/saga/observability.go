...
func (m *Monitor) WritePrometheus(ctx context.Context, w io.Writer) error {
	...
	for _, definition := range []struct{ name, help string }{
		...
		{"bahia_openclaw_provisioning_dm_gate", "Whether the governed readiness stage has completed"},
		{"bahia_openclaw_provisioning_terminal_projection", "Whether stage-specific terminal lineage is durable: active Soul for running, error result otherwise"},
		{"bahia_openclaw_provisioning_false_running", "Running checkpoints missing readiness or terminal lineage"},
