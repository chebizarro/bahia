...
func TestIsBahiaProjectionKind(t *testing.T) {
	testCases := []struct {
		...
		{"assistant transcript", AssistantTranscript, true},
		...
		{"SBOM reference", SBOMReference, true},
		{"SBOM availability list", SBOMAvailabilityList, true},
		{"continuity heartbeat observation", HeartbeatObservation, true},
		{"continuity status", ContinuityStatus, true},
		{"continuity degraded mode activation", DegradedModeActivation, true},
		{"continuity recovery progress", RecoveryProgress, true},
		{"continuity profile", ContinuityProfile, true},
		{"continuity failover policy", FailoverPolicy, true},
		{"continuity standby node", StandbyNodeDefinition, true},
		{"continuity replication policy", ReplicationPolicy, true},
		{"continuity recovery workflow", RecoveryWorkflow, true},
		{"legacy SBOM index", LegacySBOMIndex, false},
...
func TestIsOpenInteropKind(t *testing.T) {
	testCases := []struct {
		...
		{"SoulFactory action", SoulFactoryAction, true},
		{"SoulFactory runtime capability", SoulFactoryRuntimeCapability, true},
		{"SoulFactory runtime control", SoulFactoryRuntimeControl, true},
