...
func assistantAsyncToolDefinitions() []Tool {
	...
	return []Tool{
		...
		{Name: "bahia_assistant_llm_deploy", Description: "Assistant-safe async LLM deploy command (ContextVM llm/deploy request)", InputSchema: obj(map[string]interface{}{"route_id": id, "environment_id": id, "release_id": id, "requested_by": id, "idempotency_key": id}, "route_id", "environment_id", "release_id", "idempotency_key")},
		{Name: "bahia_assistant_llm_approve_deployment", Description: "Assistant-safe async LLM approval command (ContextVM approval/llm-approve or approval/llm-reject request, by decision)", InputSchema: obj(map[string]interface{}{"intent_id": id, "decision": id, "idempotency_key": id}, "intent_id", "decision", "idempotency_key")},
		{Name: "bahia_assistant_llm_rollback", Description: "Assistant-safe async LLM rollback command (ContextVM llm/rollback request)", InputSchema: obj(map[string]interface{}{"route_id": id, "environment_id": id, "requested_by": id, "idempotency_key": id}, "route_id", "environment_id", "idempotency_key")},
		{Name: "bahia_assistant_ml_deploy", Description: "Assistant-safe async ML deploy command (ContextVM ml/inference-deploy request)", InputSchema: obj(map[string]interface{}{"endpoint": id, "endpoint_id": id, "model_version": id, "model_version_id": id, "runtime_preference": id, "runtime": id, "placement": map[string]interface{}{"type": "object"}, "tags": map[string]interface{}{"type": "object"}, "idempotency_key": id}, "idempotency_key")},
		{Name: "bahia_assistant_ml_approve_deployment", Description: "Assistant-safe async ML approval command (ContextVM ml/inference-approval request)", InputSchema: obj(map[string]interface{}{"intent_id": id, "decision": id, "tags": map[string]interface{}{"type": "object"}, "idempotency_key": id}, "intent_id", "decision", "idempotency_key")},
		{Name: "bahia_assistant_ml_rollback", Description: "Assistant-safe async ML rollback command (ContextVM ml/inference-rollback request)", InputSchema: obj(map[string]interface{}{"endpoint": id, "endpoint_id": id, "requested_by": id, "tags": map[string]interface{}{"type": "object"}, "idempotency_key": id}, "idempotency_key")},
	}
}
